...
@register(task="sanity", dataset="demand.egon_cts_heat_demand_building_share", 
          rule_id="CTS_HEAT_DEMAND_SHARE", kind="sanity", rtol=1e-5, scenario_col="scenario")
class CtsHeatDemandShare(SqlRule):
    """
    ...
    to all buildings. Matches cts_heat_demand_share() from eGon-data.
    """
    def sql(self, ctx):
    ...
    def postprocess(self, row, ctx):
        ...
        if ok:
            message = f"All {total_pairs} bus/scenario pairs have heat demand shares summing to 1.0 within tolerance {rtol}"
        else:
