    ...
    * IGGIELGN_LNGs.csv (LNG terminals)
    ...
    with zipfile.ZipFile(zip_file, 'r') as zip_ref:
        for comp in components:
            csv_filename = f"data/IGGIELGN_{comp}.csv"  # Files are in data/ subdirectory
            try:
                # Extract specific file
                zip_ref.extract(csv_filename, extract_dir.parent)  # Extract to parent to preserve structure
    ...
    logger.info(f"Successfully extracted {len(extracted_files)} gas data files: {extracted_files}")
    return extract_dir
