...
def load_saved_table_count(ctx) -> int:
    ...
    try:
        import os
        import json
        metadata_file = os.path.join(ctx.out_dir, ctx.run_id, "tasks", "db_metadata.json")
...
def calculate_coverage_stats(collected_data: Dict, ctx=None) -> Dict:
    ...
    table_coverage_percent = (validated_tables_count / total_tables * 100) if total_tables > 0 else 0
    ...
    rule_coverage_percent = (applied_rules_count / total_rules * 100) if total_rules > 0 else 0
