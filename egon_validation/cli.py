...
def _run_task(args):
    try:
        ...
        if not db_url:
            ...
            raise SystemExit(
                "Missing DB URL (use --db-url, set EGON_DB_URL, or "
                "configure .env file)"
            )
        ...
        logger.warning(
            ...
            extra={"task": args.task, "run_id": args.run_id},
        )
        raise SystemExit(1)
...
def _final_report(args):
    try:
        ctx = RunContext(
            run_id=args.run_id, scenario=None, out_dir=args.out, extra={}
        )
        logger.info(
            "Starting final report generation",
            extra={"run_id": args.run_id, "output_dir": args.out},
        )
        ...
        logger.error(
            "Required validation data not found",
            extra={
                "run_id": args.run_id,
                "error": str(e),
                "suggestion": (
                    "Ensure run-task was completed successfully first"
                ),
            },
        )
...
def main():
    ...
    p1.add_argument(
        ...
        type=str,
        default="pipeline",
        choices=["pipeline", "dev"],
        help="Set log format style",
    )
    ...
    log_level = getattr(args, "log_level", None) or os.getenv(
        "EGON_LOG_LEVEL", "INFO"
    )
    log_format = getattr(args, "log_format", None) or os.getenv(
        "EGON_LOG_FORMAT", "pipeline"
    )
