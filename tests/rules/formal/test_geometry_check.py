from egon_validation.rules.formal.geometry_check import (
...
class TestGeometryContainmentValidation:
    def test_sql_generation_default_parameters(self):
        ...
        assert (
            "ST_Transform(points.geom, 3035)" in sql
    ...
    def test_postprocess_wind_plants_debugging_info(self):
        ...
        rule = GeometryContainmentValidation(
            "WIND_PLANTS_IN_GERMANY", "test_task", "test.table"
        )
    ...
    def test_postprocess_non_wind_plants_no_debugging_info(self):
        ...
        rule = GeometryContainmentValidation(
            "OTHER_RULE", "test_task", "test.table"
        )
        ...
        assert (
            "SELECT * FROM supply.egon_power_plants_wind" not in result.message
        )
