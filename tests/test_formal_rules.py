...
from egon_validation.rules.formal.data_type_check import DataTypeValidation, MultipleColumnsDataTypeValidation
...
class TestDataTypeValidation:
    def test_sql_generation_with_schema(self):
        rule = DataTypeValidation("test_rule", "test_task", "schema.table", 
                                column="year", expected_type="integer")
        sql = rule.sql(None)
    ...
    def test_sql_generation_without_schema(self):
        rule = DataTypeValidation("test_rule", "test_task", "table_only", 
                                column="year", expected_type="integer")
        sql = rule.sql(None)
    ...
    def test_postprocess_type_mismatch(self):
        rule = DataTypeValidation("test_rule", "test_task", "test.table",
                                column="name", expected_type="integer")
        row = {
            "column_name": "name",
            "data_type": "text",
            "udt_name": "text"
    ...
    def test_type_mappings(self):
        ...
        # Test text types
        row = {"column_name": "name", "data_type": "character varying", "udt_name": "varchar"}
...
class TestMultipleColumnsDataTypeValidation:
    def test_sql_generation(self):
        rule = MultipleColumnsDataTypeValidation(
            "test_rule", "test_task", "schema.table",
...
class TestRange:
    ...
    def test_sql_generation_custom_params(self):
        rule = Range("test_rule", "test_task", "test.table", 
                    column="price", min_val=10.5, max_val=100.0)
        sql = rule.sql(None)
...
class TestValueSetValidation:
    def test_sql_generation(self):
        rule = ValueSetValidation("test_rule", "test_task", "test.table",
                                column="status", expected_values=["active", "inactive"])
        sql = rule.sql(None)
